...
def get_nifty_top_n(n: int = 400) -> list:
    """
    Attempt to load a top-n list (e.g., Nifty Top 400) from a local CSV file named
    `nifty_top_400.csv`. If the file is not found, fall back to the Nifty50 list.

    CSV format expected: one column with header 'Symbol' or the first column as tickers
    and symbols should be in Yahoo format (e.g., 'RELIANCE.NS').
    """
    # Look for file in repo root or data/ directory
    candidates = ["nifty_top_400.csv", os.path.join("data", "nifty_top_400.csv")]
